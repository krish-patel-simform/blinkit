...
export default function SearchProductPage() {
  ...
  const filterProducts:ProductCategory = useMemo(()=>{
    if(query)
    {
      ...
      {
        products.push(...category.products.filter((product)=> product.name.toLowerCase().includes(query)))
      }
      ...
      <div className="grid grid-cols-6 gap-4 p-8!">
        {
          filterProducts.products.map((product)=>{
            return <ProductCard
              ...
              quantity={selectedProducts.find(
...
function NoProductFound()
{
  return (
    <div className="w-full h-full flex items-center justify-center flex-col">
        <img className="h-60 aspect-square" src="https://blinkit.com/57070263a359a92dc0fe.png"  alt="No Product Found"/>
